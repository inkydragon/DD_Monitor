...
class MainWindow(QMainWindow):
    ...
    def __init__(self, cacheFolder, progressBar, progressText):
        ...
        self.hotKeyMenu = self.menuBar().addMenu('快捷键')
        hotKeyAction = QAction('所有快捷键', self, triggered=self.openHotKey)
        self.hotKeyMenu.addAction(hotKeyAction)
        controlPanelAction = QAction('显示 / 隐藏控制条(H)', self, triggered=self.openControlPanel)
        self.hotKeyMenu.addAction(controlPanelAction)
        self.fullScreenAction = QAction('全屏(F) / 退出(Esc)', self, triggered=self.fullScreen)
        self.hotKeyMenu.addAction(self.fullScreenAction)
        progressText.setText('设置快捷键菜单...')
